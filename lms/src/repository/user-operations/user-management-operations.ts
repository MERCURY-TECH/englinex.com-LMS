...
const findUserByID:ILogic = {
    ...
    callback: async function (userId:String) {
       return await User.findOne({_id:userId}).populate(['registeredCourses']).select(['-password', "-isRoot","-isSuspended"]);
    }
}
...
const findUserByField:ILogic = {
    ...
    callback: async function (collection:any) {
       return await User.findOne(collection).populate(['registeredCourses']).select(['-password', "-isRoot","-isSuspended"]);
    }
}
...
const getAllUsers:ILogic = {
    ...
    callback: async function () {
       return await User.find({isRoot:false}).populate(['registeredCourses']).select(['-password', "-isRoot","-isSuspended"]);
    }
}
...
const getAllUsersPerAccountType:ILogic = {
    ...
    callback: async function (accountType:string) {
        ...
        let users = await User.find({isRoot:false,accountType: accountType}).populate(['registeredCourses']).select(['-password', "-isRoot","-isSuspended"]);
       return users;
    }
}
...
const createTeacherAccount:ILogic = {
    ...
    callback: async function (collection:IUser) {
       ...
       return await new User(collection).save().then((t:any)=> t.populate(['registeredCourses']).select(['-password', "-isRoot","-isSuspended"]));
    }
}
